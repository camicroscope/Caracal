...
General.get = function (db, collection) {
  return function (req, res, next) {
    const query = req.query;
    delete query.token;
    mongoFind(db, collection, { _id: req.query.id })
      .then((x) => {
        req.data = x;
        next();
      })
...
General.distinct = function (db, collection, upon) {
  return function (req, res, next) {
    const query = req.query;
    delete query.token;
    mongoDistinct(db, collection, upon, query)
      .then((x) => {
        req.data = x;
        next();
      })
...
General.update = function (db, collection) {
  return function (req, res, next) {
    ...
    mongoUpdate(db, collection, query, newVals)
      .then((x) => {
        req.data = x;
        next();
      })
...
General.delete = function (db, collection) {
  return function (req, res, next) {
    const query = req.query;
    delete query.token;
    mongoDelete(db, collection, query)
      .then((x) => {
        req.data = x;
        next();
      })
...
Presetlabels.add = function (req, res, next) {
  ...
  mongoUpdate('camic', 'configuration', { config_name: 'preset_label' }, { $push: { configuration: labels } })
    .then((x) => {
      req.data = x;
      next();
    })
...
Presetlabels.remove = function (req, res, next) {
  ...
  mongoUpdate(
    ...
    { $pull: { configuration: { id: query.id } } }
  )
    .then((x) => {
      req.data = x;
      next();
...
Mark.multi = function (req, res, next) {
  ...
  const postQuery = JSON.parse(req.body);
...
Mark.findMarkTypes = function (req, res, next) {
  ...
  if (query['provenance.analysis.source'] == 'human') {
    ...
    mongoAggregate('camic', 'mark', pipeline)
      .then((x) => {
        req.data = x;
        next();
      })
    ...
    mongoDistinct('camic', 'mark', 'provenance.analysis', query)
      .then((x) => {
        req.data = x;
        next();
      })
...
User.wcido = function (req, res, next) {
  ...
  if (DISABLE_SEC || userType == 'Admin') {
    ...
    const error = { error: 'undefined UserType' };
    res.send(error);
  }
};
