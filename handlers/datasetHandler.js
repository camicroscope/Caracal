...
class Data {
  ...
  async load() {
    ...
    this.trainImages = this.datasetImages.slice(0, this.IMAGE_SIZE * this.NUM_TRAIN_ELEMENTS * this.NUM_CHANNELS);
    this.testImages = this.datasetImages.slice(this.IMAGE_SIZE * this.NUM_TRAIN_ELEMENTS * this.NUM_CHANNELS);
  ...
  nextBatch(batchSize, data, index) {
    const batchImagesArray = new Float32Array(batchSize * this.IMAGE_SIZE * this.NUM_CHANNELS);
    ...
    this.xs = tf.tensor3d(batchImagesArray, [batchSize, this.IMAGE_SIZE, this.NUM_CHANNELS]);
...
function getDataset() {
  return function (req, res) {
    ...
    fs.writeFile('dataset/' + userFolder + '/dataset.zip', data.file, { encoding: 'base64' }, async function (err) {
